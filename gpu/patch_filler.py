...
STATE_DIRS = [Path(d) for d in os.getenv("STATES_DIRS", "/workspace/states:/root/states2").split(":")]
RESULTS = Path(os.getenv("RESULTS_FILE", "/root/results/patch_results.jsonl"))
HEARTBEAT = Path(os.getenv("HEARTBEAT_FILE", "/root/heartbeat_patch"))
DONE = Path(os.getenv("DONE_MARKER", "/root/patch.done"))
CONDITIONS = tuple(os.getenv("CONDITIONS", "false,counting").split(","))
PER_CONDITION = 100
N_SANITY = 5
